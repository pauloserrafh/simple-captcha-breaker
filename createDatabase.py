...
def deCaptcha(imageset, vector):
  ...
  for i in range(5):
    im3 = Image.open('images/'+ str(i+1) +'.gif')
    guess = []
...
def find_letters(image):
  ...
  for letter in letters:
    ...
    arr = numpy.array(im3)
    imsave('trainingset/'+str(i+1)+'.gif', im3)
    i += 1
...
if __name__ == "__main__":
  ...
  for j in range(4,5):
    print("\n===========================================================\n")
    image_in = image_folder + image_name + str(j) + image_ext
    image = remove_background(image_in)
    image = black_chars(image)
    find_letters(image)
    # result = deCaptcha(imageset, v)

    #Saving the images in the correct folders
    for i in range(5):
        print ("If you CAN recognize the character from the croped image press \
it.")
        print ("If you CANNOT recognize the character from the croped image \
press N")
        text = "Is the character " + str(i+1) + " from captcha" + str(j) + \
".gif recognizable? "
        a = raw_input(text)
        if (a == 'N'):
          print ("Error on letter " + str(i+1) + " on captcha" + str(j))
        else:
          if not os.path.isdir('iconset/'+ a):
            os.makedirs('iconset/'+ a)
          m = hashlib.md5()
          m.update("%s%s%s"%(time.time(), j, i))
          shutil.move('trainingset/'+ str(i+1) +'.gif', 'iconset/%s/%s.gif'\
        %(a, m.hexdigest()))
